#    RECORD FOR M3AXPIV2 -> WHICH HAVE capture_for_ai() FUNCTION
...
# Tambahkan path ke folder yang ada m3axpiv2.so
sys.path.append("/root/librarychange/axpipeline/ax-pipeline-api")

try:
    import m3axpiv2
except ImportError as e:
    print(f"❌ Gagal import m3axpiv2: {e}")
    exit(1)

# Folder hasil simpan
SAVE_DIR = "/root/capture_npu"
os.makedirs(SAVE_DIR, exist_ok=True)

async def stream_camera():
    # Inisialisasi kamera
    try:
        m3axpiv2.camera(SysCase=2)
        m3axpiv2.load("/home/config/yolov8.json")
        time.sleep(1)
        print("✅ Kamera siap")
    ...
    while True:
        try:
            # frame_npu = m3axpiv2.capture()
            frame_npu = m3axpiv2.capture_for_ai()
